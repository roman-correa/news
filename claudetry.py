...
MEDELLIN_BOUNDS = dict(lat_min=6.175, lat_max=6.38,
                       lon_min=-75.67, lon_max=-75.51)
...
def fix_coordinates(df: pd.DataFrame, geo: gpd.GeoDataFrame) -> pd.DataFrame:
    ...
    if bad.sum():
        centroids = geo.set_index("barrio_norm")[["centroid_x", "centroid_y"]]
        df.loc[bad, "longitud"] = df.loc[bad, "barrio"].map(
            centroids["centroid_x"])
        df.loc[bad, "latitud"] = df.loc[bad, "barrio"].map(
...
def clean(df: pd.DataFrame, geo: gpd.GeoDataFrame, operacion: str) -> pd.DataFrame:
    ...
    df = df[["tipo", "precio", "area", "habitaciones", "baños",
             "parqueaderos", "nombre", "latitud", "longitud"]].dropna(subset=["nombre"])
...
def train_xgb(X_train: pd.DataFrame, y_train: pd.Series,
    ...
    model = XGBRegressor(
        n_estimators=1000,          # high ceiling — early stopping will trim
        learning_rate=0.05,
        max_depth=5,
        subsample=0.8,
        colsample_bytree=0.8,
        min_child_weight=5,         # reduces overfitting on small groups
...
def run_pipeline(skip_scrape: bool = False, mode: str = "both"):
    ...
    def prep_dataset(df, label):
        ...
        best_cols = select_features(X_tr_t, y_tr, max_features=12)
